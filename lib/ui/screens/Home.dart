...
class _HomeState extends State<Home> {
  ...
  RssFeed _rssFeed; // RSS Feed Object
  ...
  Future<RssFeed> getMediumRSSFeedData() async {
    try {
      ...
      final response = await client.get(MEDIUM_PROFILE_RSS_FEED_URL);
    ...
    getMediumRSSFeedData().then((feed) {
      ...
      for(RssItem rssItem in feed.items) {
        ...
        if(rssItem.description != null) {
          ...
          MediumArticle mediumArticle = MediumArticle(title: rssItem.title, link: rssItem.link, datePublished: rssItem.pubDate.toString());
        ...
        itemBuilder: (BuildContext buildContext, int index) {
          return Container(
            child: ListTile(
              ...
              onTap: () => launchArticle(_mediumArticles[index].link),
